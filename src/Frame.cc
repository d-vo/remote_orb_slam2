...
Frame::Frame(const FrameInfo &info, const std::vector<cv::KeyPoint> &keyPoints, const cv::Mat &descriptors,
		const std::vector<unsigned int> &visualWords, const double &timeStamp, ORBextractor* extractor,
 ...
 mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
	...
	cv::Mat imGray(info.mnHeight, info.mnWidth, CV_8U, cv::Scalar::all(0));
